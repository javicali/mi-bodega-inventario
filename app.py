...
def guardar_cambio_google(sh, tab, accion, datos):
    try:
        ...
        if accion == "UPDATE_STOCK":
            # Cambiado para buscar por ID combinado para evitar errores entre tiendas
            todo = ws.get_all_values()
            bod_b, cod_b = datos[0].split("_")[0], datos[0].split("_")[-1]
            for i, fila in enumerate(todo[1:], start=2):
                if str(fila[1]).strip().upper() == bod_b.upper() and str(fila[2]).strip().upper() == cod_b.upper():
                    ws.update_cell(i, 4, datos[1])
                    break
        elif accion == "ADD_LOG":
...
def mostrar_tarjeta(k, v, suf):
    ...
    with st.container(border=True):
        ...
        c2.write(f"📦 {v['stock']}")
        with c3:
            # Vinculado al session_state para permitir el reseteo
            ...
            if cols_btn[0].button("ENTRÓ", key=f"btn_add_{suf}_{k}", use_container_width=True): 
                ...
                else: st.error("Ingresa una cantidad")
            if cols_btn[1].button("SALIÓ", key=f"btn_sub_{suf}_{k}", disabled=v['stock']<cant or cant == 0, use_container_width=True): 
