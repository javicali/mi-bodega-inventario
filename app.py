    ...
    try:
        ...
        if accion == "UPDATE_STOCK":
            ...
            if celda:
                ...
                st.error(f"❌ No se encontró el código '{codigo_puro}' en el Excel.")
                return False
            ...
            ws.append_row(datos)
            st.toast(f"✅ Nuevo código '{datos[2]}' creado", icon="🆕")
            return True
...
def confirmar_mov(k, v, cant, op):
    ...
    if c1.button("SÍ, GUARDAR", use_container_width=True):
        ...
        if guardar_cambio_google(sh, "INVENTARIO", "UPDATE_STOCK", [k, nuevo_val]):
            ...
            st.toast(f"📦 Stock actualizado: {k.split('_')[-1]}", icon="✅")
            recargar()
            st.rerun()
...
if st.session_state.get('ver_historial'):
    ...
    # VISTA PANEL DE TRABAJO (EDICIÓN)
    ...
    if bus_ed:
        for k, v in {k: v for k, v in inv.items() if bus_ed in k}.items():
            with st.container(border=True):
                ...
                with c3:
                    cant = st.number_input("n", min_value=1, key=f"n_ed_{k}", label_visibility="collapsed")
                    cb1, cb2 = st.columns(2)
                    if cb1.button("➕", key=f"add_{k}"): confirmar_mov(k, v, cant, "SUMAR")
                    if cb2.button("➖", key=f"sub_{k}", disabled=v['stock']<cant): confirmar_mov(k, v, cant, "RESTAR")
...
with st.sidebar:
    ...
    if not st.session_state.get('edit_mode'):
        ...
        with st.expander("🆕 Crear Nuevo Artículo"):
            nma = st.selectbox("Marca", config["marcas"], key="new_ma")
            nco = st.text_input("Código", key="new_co").upper().strip()
            nbo = st.selectbox("Bodega", config["depositos"], key="new_bo")
        ...
        if st.session_state.usuario_actual.upper() == "ADMIN":
            ...
            with st.expander("🏷️ Gestión Marcas"):
                ms = st.selectbox("Borrar Marca:", config["marcas"])
                if st.button("🗑️ Borrar Marca"):
                    guardar_cambio_google(sh, "CONFIG", "DEL_CONFIG", [ms, 4])
                    recargar(); st.rerun()
            ...
            with st.expander("🏘️ Gestión Bodegas"):
                bs = st.selectbox("Bodega:", config["depositos"])
                nb_n = st.text_input("Nuevo nombre:").upper().strip()
                if st.button("📝 Renombrar Bodega"):
                    guardar_cambio_google(sh, "CONFIG", "RENAME_CONFIG", [bs, nb_n, 3])
                    recargar(); st.rerun()
